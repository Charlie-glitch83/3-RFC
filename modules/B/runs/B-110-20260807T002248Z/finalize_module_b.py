...
handoff={
 ...
 'first_physical_event':primary['event'],
 'carrier':primary['carrier'],
 'operator':primary['operator'],
 'physical_state':primary['state']['post_event_physical_state'],
 'pregeometry':primary['pregeometry'],
 'sector_seed_status':primary['sector_seed_status'],
 'no_loss':primary['no_loss'],
 'uncertainty':load(RUN/'primary/UNCERTAINTY_ENVELOPE.json'),
 'restart':load(RUN/'CHECKPOINT_RECORD.json'),
 'independent_verification':{'method':indep['method'],'pass':indep['pass'],'comparisons':indep['comparisons']},
...
dump(RUN/'IMPLEMENTATION_CORRECTION_LEDGER.json',{'run_id':RUN_ID,'status':'FINAL','frozen_science_changed_after_lock':False,'records':[{'id':'B-110-PRELOCK-EDGE-NORMALIZATION-DIAGNOSTIC','phase':'PRE_LOCK','scientific_evidence':False,'definition_change_after_lock':False},{'id':'B-110-PRELOCK-UNIT-EDGE-DIAGNOSTIC','phase':'PRE_LOCK','scientific_evidence':False,'definition_change_after_lock':False}],'post_lock_implementation_corrections':[]})
...
# Finalize manifest only after all scientific/evidence outputs above have stopped changing.
paths=[]
for p in sorted(RUN.rglob('*')):
    if not p.is_file(): continue
    if '__pycache__' in p.parts or p.name=='GENERATED_OUTPUT_MANIFEST.json': continue
    paths.append(entry(p))
for p in [ROOT/'modules/B/frozen/H_B_to_C.json',ROOT/'modules/B/frozen/H_B_to_C_MANIFEST.json']:
    paths.append(entry(p))
