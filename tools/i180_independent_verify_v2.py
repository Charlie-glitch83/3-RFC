...
def mm(A,B): return np.asarray(A,float)@np.asarray(B,float)
def main():
 p,b,c=load(P),load(B),load(C)
 # Independent reconstruction reads no I primary artifact, GATE_RESULTS, CLOSEOUT, or OUTPUT contract.
 ...
 # Reconstruct K3 algebra, Green/resistance no-loss identity, and spectrum from first principles.
 ...
 # Counterexample arithmetic, independently reproduced.
 def Rvals(a,b,c):
