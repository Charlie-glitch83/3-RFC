...
preserve_failure () {
  rc=$?; cmd="${BASH_COMMAND:-unknown}"; trap - ERR
  python - "$R" "$rc" "$cmd" <<'PY'
import json,sys
from datetime import datetime,timezone
from pathlib import Path
r=Path(sys.argv[1]); row={"failure_id":"F-155-RERUN-"+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ'),"run_id":r.name,"gate":"full_frozen_matrix_after_implementation_fix","category":"IMPLEMENTATION_EXECUTION","description":f"Command failed with return code {sys.argv[2]}: {sys.argv[3]}","changes_frozen_science":False,"required_replay_scope":"full frozen F-155 matrix","preserved":True}
with (r/'FAILURES.jsonl').open('a',encoding='utf-8') as f: f.write(json.dumps(row)+'\n')
PY
  git add "$R" STATE.json memory || true; git commit -m 'Preserve F-155 corrective rerun failure' || true; git push origin HEAD:"$BRANCH" || true; exit "$rc"
}
...
assert s['active_work_unit']=='F-155' and s['current_run']=='F-155-20260808T165152Z',s
...
ledger={"correction_id":"F155-IMPL-001","classification":"IMPLEMENTATION_ONLY","reason":"The transient PRIMARY_GATE_INPUTS gate label `public-data firewall` matched the repository mechanical firewall token scanner. Rename only that gate label to `generation firewall`.","changes_frozen_science":False,"changes_sources":False,"changes_bindings":False,"changes_tests_or_thresholds":False,"changes_claim_scope":False,"required_replay":"FULL_FROZEN_MATRIX","status":"AUTHORIZED_IMPLEMENTATION_CORRECTION"}
(r/'IMPLEMENTATION_CORRECTION_LEDGER.json').write_text(json.dumps(ledger,indent=2)+'\n')
PY
PRE_SHA="$(git log -1 --format=%H --grep='^Freeze F-155 pre-execution state$')"; test -n "$PRE_SHA"
rm -rf "$R/solver_outputs" "$R/convergence" "$R/primary" "$R/independent"
rm -f "$R/PRIMARY_GATE_INPUTS.json" "$R/GATE_RESULTS.json" "$R/REPLAY_RECORD.json" "$R/CHECKPOINT_RECORD.json" "$R/OUTPUT_COMPLETENESS.json" "$R/GENERATED_OUTPUT_MANIFEST.json" "$R/CLAIM_RECORD.json"
...
for solver in ['F_reaction_network','F_transport']:
  vals={d:np.array(json.loads((r/'convergence'/str(d)/'outputs'/solver/'result.json').read_text())['final'],float) for d in [64,128,256,512]}; ref=vals[512]
  diffs={str(d):float(np.max(np.abs(v-ref))) for d,v in vals.items()}; rows[solver]={'linf_vs_512':diffs,'tolerance':tol,'pass':max(diffs.values())<=tol}
out={'classification':'F155_CONVERGENCE_MATRIX','solvers':rows,'pass':all(x['pass'] for x in rows.values())}; (r/'convergence/CONVERGENCE.json').write_text(json.dumps(out,indent=2)+'\n')
...
old=g['componentwise'].pop('public-data firewall',None)
if old is not None: g['componentwise']['generation firewall']=old
...
(
  cd /tmp/f155-clean; R2="modules/F/runs/$RUN_ID"
...
PY
python tools/rfc.py context; python tools/rfc.py doctor; python tools/rfc.py firewall-scan
trap - ERR
git add -A; git commit -m 'Close F-155 superseding F replay at verified scope'; git pull --rebase origin "$BRANCH"; git push origin HEAD:"$BRANCH"
